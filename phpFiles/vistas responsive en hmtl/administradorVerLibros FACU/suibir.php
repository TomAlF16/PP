...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($titulo) || empty($precio) || empty($descripcion) || empty($precioAlquiler) || empty($stockVenta) || empty($stockAlquiler) || empty($nombreAutor)) {
        ...
        $sql = "INSERT INTO libro (titulo, PrecioVenta, PrecioAlquiler, StockVenta, StockAlquiler, NombreAutor, Descripcion, imagen) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        // Preparar la declaración
        if ($stmt = $conn->prepare($sql)) {
            // Enlazar los parámetros
            $stmt->bind_param("ssssssss", $titulo, $precio, $precioAlquiler, $stockVenta, $stockAlquiler, $nombreAutor, $descripcion,$rutaDestino);
