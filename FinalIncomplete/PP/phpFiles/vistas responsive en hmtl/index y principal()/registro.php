...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $NumeroTarjeta = $_POST['tarjeta'];  // Número de tarjeta

    // Encriptar la contraseña
    $passwordHash = password_hash($password, PASSWORD_DEFAULT);

    // Conexión a la base de datos
    $conn = new mysqli('localhost', 'root', '', 'base4');
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            $insertCliente = "INSERT INTO cliente (idUsuario, nombre, NumeroTarjeta) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($insertCliente);
            if ($stmt === false) {
                die("Error en la preparación de la consulta de cliente: " . $conn->error);
            }

            // Usamos el nombre del cliente, el idUsuario y el número de tarjeta
            $stmt->bind_param("iss", $idUsuario, $nombreCliente, $NumeroTarjeta);
            if ($stmt->execute()) {
            ...
            <div id="inicio">
                <form action="registro.php" method="POST">
                    <div id="formulario">
                        ...
                        <input class="input" type="text" name="tarjeta" placeholder="Número de Tarjeta" required>
